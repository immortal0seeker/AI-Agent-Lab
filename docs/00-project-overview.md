...
foundation contracts. `P2-M2-S1` through `P2-M2-S3` add the first executable
built-in Tool: bounded workspace-relative UTF-8 reads, safe failure results,
explicit truncation metadata, and caller-controlled Registry initialization.
`list_dir`, Provider tool calling, the Agent Loop, Agent APIs, and frontend
Agent/ToolCall views remain deferred.

The next batch is `P2-M2-S4` through `P2-M2-S6`. See the
