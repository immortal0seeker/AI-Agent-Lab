...
当前开发阶段：Plan 2 M5 封版候选材料已完成到 `P2-M5-S6`。当前正式 tag 仍是
`v0.1.0`；Plan 2 最终 review 与 `v0.2.0` tag 仍留在 S7～S8。
...
下一批：`P2-M5-S7` 到 `P2-M5-S8`，执行 Plan 2 最终 review、修复、tag 和
Plan 3 桥接决策。
...
以上均为只使用合成 ID 的本地脱敏 Mock 演示，没有读取项目后端数据库。它们用于
准备 `v0.2.0` 材料，但不表示 S7～S8 最终 review 或 tag 已经完成。
...
[Plan 2 封版候选说明](docs/13-plan-2-basic-agent.md)。
