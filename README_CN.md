...
当前最新 Git tag：`v0.1.0`（Plan 1 工程底座）。Plan 2 的 `v0.2.0`
封版候选已通过 Codex final review，但仍等待用户手动创建 release commit 和
annotated tag。
...
当前开发阶段：Plan 2 实现与最终 review 已完成到 `P2-M5-S7`，进入 Plan 3 的
五项桥接契约已重新验证。`P2-M5-S8` 只剩用户创建 `v0.2.0` release commit/tag，
以及随后核对 tag target。
...
桌面端/移动端封版候选截图；这些检查没有修改前端运行时行为。

下一步：用户手动提交本次封版候选、创建 annotated tag `v0.2.0`，再核对 tag
target；通过后才能开始 `P3-M1-S1`。
...
以上均为只使用合成 ID 的本地脱敏 Mock 演示，没有读取项目后端数据库。S7 最终
review 已通过；这些图片不表示用户负责的 `v0.2.0` tag 已经存在。
...
- [Plan 2 基础 Agent 封版候选说明](docs/13-plan-2-basic-agent.md)
...
验收。tracked 模型未开启 Tool 能力，Agent Provider 调用尚未关联 `LLMCall`
usage/cost 记录，`web_fetch` 也继续明确延期且没有运行时表面。完整限制见
[Plan 1 工程底座封版说明](docs/02-plan-1-foundation.md)、
[Agent API](docs/12-agent-api.md)和
[Plan 2 封版候选说明](docs/13-plan-2-basic-agent.md)和
[Plan 2 最终复审记录](docs/reviews/2026-07-19-plan2-v0.2.0-final-review.md)。
