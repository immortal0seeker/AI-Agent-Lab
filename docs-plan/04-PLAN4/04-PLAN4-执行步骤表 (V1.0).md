...
> 审查策略（2026-07-18 用户决定）：每批只执行 Codex self-review；外部复审不作为验收或推进门槛。全部 6 个 Plan 和整个项目完成后，再由用户决定是否使用 Fable 5 做一次全项目检查。
...
| Codex self-review 时机 | 每个 1～3 Step 批次完成后；里程碑结束时执行汇总审查 |
...
|---|---|---|---|---:|---|---|
| Phase 1 | M1 交接与 Trace 基础设施 | Step 1～3 | v0.3.0 检查、TraceRun / TraceStep、Trace Service / Context | 20～30 h | Codex | Codex self-review |
| Phase 2 | M2 Trace 接入与 Timeline | Step 4～7 | LLM Trace、RAG Trace、Trace API、前端 Timeline | 25～40 h | Codex + Cursor | Codex self-review |
| Phase 3 | M3 Advanced Retrieval 基础 | Step 8～10 | Metadata Filtering、BM25 / Keyword Search、Hybrid Search、RRF | 25～40 h | Codex | Codex self-review |
| Phase 4 | M4 Retrieval 扩展策略 | Step 11～12 | Parent-Child Retrieval、Query Rewrite | 20～35 h | Codex | Codex review |
| Phase 5 | M5 Rerank + Strategy + Advanced API | Step 13～19 | Rerank Provider、Strategy Registry、Advanced RAG API、Strategy Compare、前端策略页 | 35～55 h | Codex + Cursor | Codex self-review |
| Phase 6 | M6 Evaluation 闭环 | Step 20～26 | Eval 数据模型、Dataset API、Runner、指标、Evaluation API、前端 Eval 页面 | 35～55 h | Codex + Cursor | Codex self-review |
| Phase 7 | M7 测试、文档与封版 | Step 27～29 | 测试、README、docs、截图、CHANGELOG、v0.4.0 tag | 10～20 h | Codex + Cursor | Codex self-review |
...
| Batch 5 | P4-M2-S4～S6 | 接入 RAG 检索 Trace 和候选记录 | RAG Trace 测试 | 已完成（retrieval Run/candidate 模型与 0009 迁移、Query/RAG Chat retrieval/Prompt/answer Trace、失败重放与 Codex self-review 通过） |
| Batch 6 | P4-M2-S7～S9 | 实现 Trace API 和前端 Timeline | API 测试 + 浏览器手测 | 已完成（只读聚合 API、第四 Trace workspace、深链/竞态/响应式测试与 Codex self-review 通过） |
| Batch 7 | P4-M2-S10 | 完成 Timeline review | Codex self-review M2 | 已完成（Timeline 指标缺口已按 TDD 修复；使用指南、全量回归、headed 浏览器验收与 Codex self-review 通过） |
| Batch 8 | P4-M3-S1～S3 | 实现 metadata filter 和策略接口基础 | Retriever 测试 | 未完成 |
| Batch 9 | P4-M3-S4～S6 | 实现 BM25 / keyword search | 搜索测试 | 未完成 |
| Batch 10 | P4-M3-S7～S9 | 实现 Hybrid Search 和 RRF 融合 | Hybrid 测试，Codex self-review M3 | 未完成 |
...
| Batch 16 | P4-M5-S10～S12 | 实现 Strategy Compare API 和前端策略页 | 浏览器手测，Codex self-review M5 | 未完成 |
...
| Batch 19 | P4-M6-S7～S9 | 实现 Evaluation API、可选 LLM-as-Judge、前端 Eval 页面 | API + 页面验证，Codex self-review M6 | 未完成 |
| Batch 20 | P4-M7-S1～S6 | 补测试、文档、截图、封版 | Codex final self-review | 未完成 |
...
| P4-M1-S2 | 设计 TraceRun / TraceStep ORM 和 schema | Codex | `trace_runs`、`trace_steps` 模型和 schema | 迁移和模型测试通过 | Codex |
...
| P4-M2-S4 | 设计 RAG retrieval run / candidate 模型 | Codex | `rag_retrieval_runs`、`rag_retrieval_candidates` | 迁移和模型测试通过 | Codex |
...
| P4-M2-S10 | 完成 M2 review 和 Trace 使用文档 | Codex | `docs/31-trace-timeline.md` | Timeline 手测记录和文档齐全 | Codex self-review |
...
| Step ID | 任务 | 建议工具 | 交付物 | 验证方式 | Review |
|---|---|---|---|---|---|
| P4-M3-S1 | 抽象 Retrieval Strategy 基类 | Codex | `rag/strategies/base.py` | naive strategy 适配测试通过 | Codex |
...
| P4-M3-S4 | 建立 keyword index 策略 | Codex | SQLite FTS5 或本地 BM25 service | 文档 chunk 可索引 | Codex |
...
| P4-M3-S9 | 将 Advanced Retrieval 写入 Trace | Codex | strategy trace writer | Trace 展示 dense、keyword、hybrid 候选 | Codex self-review |
...
| P4-M4-S2 | 实现 Parent-Child Retrieval | Codex | `parent_child.py` | 子 chunk 命中后能回查 parent content | Codex |
...
| Step ID | 任务 | 建议工具 | 交付物 | 验证方式 | Review |
|---|---|---|---|---|---|
| P4-M5-S1 | 设计 RerankProvider 抽象 | Codex | `providers/rerank/base.py` | mock rerank provider 测试通过 | Codex |
...
| P4-M5-S7 | 实现 Advanced RAG Strategy Registry | Codex | `rag/strategies/registry.py` | naive、hybrid、hybrid+rerank 策略可选 | Codex |
...
| P4-M5-S12 | 实现前端策略对比结果展示 | Cursor + Codex | compare result UI | 展示来源、分数、成本、延迟、trace 链接 | Codex self-review |
...
| Step ID | 任务 | 建议工具 | 交付物 | 验证方式 | Review |
|---|---|---|---|---|---|
| P4-M6-S1 | 设计 Evaluation 数据模型和迁移 | Codex | EvalDataset、EvalCase、EvalRun、EvalResult | 迁移和模型测试通过 | Codex |
...
| P4-M6-S4 | 实现 RAG Evaluation Runner | Codex | `evaluation/rag_runner.py` | 对固定 cases 执行指定 strategy | Codex |
...
| P4-M6-S9 | 实现前端 Evaluation 页面 | Cursor + Codex | dataset、run、results UI | 页面可查看指标和策略差异 | Codex self-review |
...
| P4-M7-S6 | Plan 4 全量 review、修复、创建 v0.4.0 tag | Codex | review 记录、修复 commit、tag、桥接检查表 | 全量测试通过，tag 存在 | Codex final self-review |
...
## 11. Codex Self-Review 节点

每个批次都执行 Codex self-review，并在这些里程碑节点做汇总审查：
...
| 验收项 | 状态 | 证据 |
|---|---|---|
| trace_runs 可用 | done | ORM / 0008 迁移 / 全量测试 |
| trace_steps 可用 | done | ORM / 0008 迁移 / 全量测试 |
| LLM 调用 Trace 可用 | done | LLM Trace 成功/失败测试 |
| RAG 检索 Trace 可用 | done | retrieval 0009 迁移 / RAG Trace 测试 |
| Trace API 可用 | done | schema / service / API 测试 |
| 前端 Trace Timeline 可用 | done | DOM 测试 / desktop-mobile headed 截图 |
| Token / Cost / Latency 可见 | done | Trace Run metrics DOM 测试 / headed 截图 |
